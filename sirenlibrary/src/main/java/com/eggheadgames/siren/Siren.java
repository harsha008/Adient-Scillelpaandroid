...
public class Siren {
    ...
    protected void handleVerificationResults(String json) {
        try {
            ...
            String packageName = getSirenHelper().getPackageName(mApplicationContext);

            if (rootJson.isNull(packageName)) {
                throw new JSONException("field not found");
            } else {
                JSONObject appJson = rootJson.getJSONObject(packageName);
